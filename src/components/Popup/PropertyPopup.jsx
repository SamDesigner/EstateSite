...
const PropertyPopup = ({ data }) => {
  ...
  return (
    <div onClick={close} className="bg-[rgba(0,0,0,0.8)]  h-full w-full fixed top-0 left-0 z-[500] flex items-center justify-center">
        <div onClick={close} className="absolute cursor-pointer top-[5%] right-[2%] bg-white text-black p-[15px] rounded-full text-[20px] z-[500]">
            <GiCancel />
        </div>
      <div onClick={(e) => e.stopPropagation()} className="bg-white rounded-[10px] h-[100vh] w-full flex items-center justify-center ">
          ...
          {data &&
            data.map((dataItem, index) => (
              <SwiperSlide key={index}>
                <img
                  className="object-cover  w-full h-[40vh] md:h-[100vh] mx-auto rounded-[10px]"
                  src={dataItem}
                />
              </SwiperSlide>
            ))}
